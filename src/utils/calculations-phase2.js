// src/utils/calculations-phase2.js
...
export function calculateOneZeroThreeOneScenario(data, replacementData, years = 10) {
  ...
  for (let year = 1; year <= years; year++) {
    ...
    const mortgagePayment = (mortgageBalance * (replacementData.replacementMortgageRate / 12)) / 
      (1 - Math.pow(1 + replacementData.replacementMortgageRate / 12, -(replacementData.replacementMortgageYears * 12 - (year - 1) * 12)));
  ...
  return {
    ...
    proceedsFromSale: round(proceedsFromSale),
    replacementValue: round(replacementValue),
    downPayment: round(downPayment),
    replacementMortgage: round(replacementMortgage),
    depreciationBasis: round(depreciationBasis),
    yearByYear,
    endEquity: round(endEquity),
    totalCashFlow: round(totalCashFlow),
    totalTaxDeferred: round(realtorFees + qiFees),
...
export function compareThreeScenarios(holdData, sellData, exchangeData, year = 10) {
  ...
  return {
    hold: round(holdTotal),
    sell: round(sellTotal),
    exchange: round(exchangeTotal),
    winner: values[0].scenario,
    topThree: values,
    advantage: round(values[0].value - values[1].value)
  };
}
